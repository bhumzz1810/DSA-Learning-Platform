...
const CodingRoom = () => {
  ...
  useEffect(() => {
    ...
    const onFilesUpdated = (updatedFiles) => {
      ...
      if (activeFileRef.current && !updatedFiles.some(f => f.path === activeFileRef.current.path)) {
        setActiveFile(null);
        setCode('');
      }
    };
  ...
  return (
    <div className={`flex flex-col h-screen ${theme === 'dark' ? 'bg-gray-900 text-white' : 'bg-gray-100 text-gray-900'}`}>
      <header className={`p-4 flex justify-between items-center border-b ${theme === 'dark' ? 'border-gray-700 bg-gray-800' : 'border-gray-200 bg-white'}`}>
        ...
        <div className="flex items-center space-x-4">
          ...
          <div className="relative group">
            <button className={`px-3 py-1 rounded-md ${theme === 'dark' ? 'bg-gray-700 hover:bg-gray-600' : 'bg-gray-200 hover:bg-gray-300'}`}>
              Theme
            </button>
            <div className={`absolute right-0 mt-2 w-32 rounded-md shadow-lg py-1 z-10 ${theme === 'dark' ? 'bg-gray-800' : 'bg-white'} hidden group-hover:block`}>
              <button 
          ...
          <button
            ...
            className={`px-4 py-2 rounded transition-colors ${theme === 'dark' ? 'bg-red-700 hover:bg-red-600' : 'bg-red-600 hover:bg-red-500'} text-white`}
          >
            Exit Session
          </button>
          ...
          <div className="flex space-x-2">
            <button
              ...
              className={`px-4 py-2 rounded transition-colors ${theme === 'dark' ? 'bg-indigo-600 hover:bg-indigo-700' : 'bg-indigo-500 hover:bg-indigo-600'} text-white`}
            >
              New File
            </button>
            <button
              ...
              className={`px-4 py-2 rounded transition-colors ${
                !activeFile || isLoading ? 'bg-gray-400 cursor-not-allowed' : theme === 'dark' ? 'bg-green-600 hover:bg-green-700' : 'bg-green-500 hover:bg-green-600'
              } text-white`}
            >
            ...
            <button
              ...
              disabled={!activeFile || isLoading}
              className={`px-4 py-2 rounded transition-colors ${
                !activeFile || isLoading ? 'bg-gray-400 cursor-not-allowed' : theme === 'dark' ? 'bg-blue-600 hover:bg-blue-700' : 'bg-blue-500 hover:bg-blue-600'
              } text-white`}
            >
              Download
            </button>
            <label className={`cursor-pointer px-4 py-2 rounded transition-colors ${
              isLoading ? 'bg-gray-400 cursor-not-allowed' : theme === 'dark' ? 'bg-purple-600 hover:bg-purple-700' : 'bg-purple-500 hover:bg-purple-600'
            } text-white`}>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className={`w-64 border-l ${theme === 'dark' ? 'border-gray-700 bg-gray-800' : 'border-gray-200 bg-white'}`}>
          <div className="h-full flex flex-col">
            ...
            <Chat 
              ...
              theme={theme}
              currentUser={alias}
              inputClassName={`${theme === 'dark' ? 'bg-gray-700 text-white border-gray-600' : 'bg-white text-gray-900 border-gray-300'}`}
            />
          </div>
        </div>
      </div>
