...
const ProblemSkeleton = () => {
  return (
    <>
      <div className="mb-6 p-4 rounded-lg bg-gray-100 animate-pulse h-28"></div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {[...Array(6)].map((_, idx) => (
          <div
            key={idx}
            className="rounded-xl p-6 bg-gray-100 h-64 animate-pulse"
          ></div>
        ))}
      </div>
    </>
...
export default function Problems() {
  ...
  const [isSubscribed, setIsSubscribed] = useState(true); // default true
  ...
  useEffect(() => {
    ...
    setTimeout(() => setIsLoading(false), 300); // slight delay for smoother feel
        ...
        {isLoading ? (
          ...
          <>
            {isLoggedIn && !isSubscribed && (
              <div className="mb-6 p-4 md:p-5 rounded-lg bg-yellow-100 border border-yellow-300 text-yellow-900 flex flex-col md:flex-row items-center justify-between gap-4">
                <div>
                  <h2 className="text-lg font-semibold">
                    🚀 Unlock Premium Features
                  </h2>
                  <p className="text-sm mt-1">
                    Get access to the Collaborative Code Editor and AI
                    Suggestions to level up your DSA skills faster.
                  </p>
                </div>
                <button
                  onClick={() => {
