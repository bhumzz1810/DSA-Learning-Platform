const ProfileCard = ({ user, leaderboard, theme }) => {
  ...
  return (
    ...
    >
      <div className="flex items-center gap-6">
        <div className="relative">
          ...
          <div
            className={`absolute -top-2 -right-2 px-2 py-1 rounded-full text-xs font-bold ${
              currentTheme.rank
            } bg-opacity-90 ${theme === "light" ? "bg-white" : "bg-gray-900"}`}
          >
            #{user.rank}
          </div>
        </div>
        <div>
          ...
          <div className="flex gap-4 mt-3">
            <button
              className={`${currentTheme.button} px-4 py-1 rounded border`}
            >
              Github
            </button>
            <button
      ...
      <button
        className={`mt-6 md:mt-0 text-sm px-4 py-2 rounded ${currentTheme.button} border`}
      >
        Edit Profile
      </button>
    </div>
